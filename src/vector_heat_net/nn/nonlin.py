...
#     def __init__(self, in_channels, nonlin=torch.sin, batchnorm=None):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x.view(int(N/2), 2, C)
        norm = torch.linalg.norm(x, dim=1, keepdim=False) # Note: torch.abs() may also be used to compute complex number norm
