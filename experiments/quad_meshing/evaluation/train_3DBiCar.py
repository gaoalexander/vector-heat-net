...
from vector_heat_net.data.retopo_dataset import RetopoDatasetV2
...
# # training settings - LARGE DATASET
...
# training settings - SMALL DATASET
# train = not args.evaluate
# n_epoch = 150000
# lr = 1e-4
# decay_every = 1000
...
DATASET = "3DBICAR"
# DATASET = "ANIMALS_GENERALIZATION_TEST"
...
if train:
    dataset_path = "/home/jovyan/git/vector-diffusion-net/experiments/quad_meshing/data/3DBiCar"
#     dataset_path = "/home/jovyan/git/vector-diffusion-net/experiments/quad_meshing/data/ICML/octopus_hi"
#     dataset_path = "/home/jovyan/git/vector-diffusion-net/experiments/quad_meshing/data/animals_generalization_test"
#     dataset_path = "/home/jovyan/git/vector-diffusion-net/experiments/quad_meshing/data/heads_mix"
#     dataset_path = "/home/jovyan/git/vector-diffusion-net/experiments/quad_meshing/data/heads_neckcap"
...
if train:
    ...
    shutil.copy2(os.path.join(base_path, "train_3DBiCar.py"), os.path.join(scripts_path, "train_3DBiCar.py"))
    ...
    train_dataset = RetopoDatasetV2(dataset_path, split='train', k_eig=k_eig, use_cache=True, op_cache_dir=None)
    train_loader = DataLoader(train_dataset, batch_size=None, shuffle=True)

test_dataset = RetopoDatasetV2(dataset_path, split='test', k_eig=k_eig, use_cache=True, op_cache_dir=None)
test_loader = DataLoader(test_dataset, batch_size=None)

# === Create the model
C_in = {'xyz': 3, 'gaussian_curvature': 2, 'mean_curvature': 2, 'principal_curvature': 6, 'hks_plus_principal': 15,
...
def train_epoch(epoch, normalize_targets=False):
    ...
    for data in tqdm(train_loader):
        ...
        if input_features == 'xyz':
            ...
            #             scalar_features_grad = scalar_features_grad / torch.mean(torch.linalg.norm(scalar_features_grad, axis=3), dim=1)
            ...
            vec_features = torch.stack((pd1, pd2, pv1_grad, pv2_grad, pv1_grad_rotated, pv2_grad_rotated),
        ...
        per_vertex_loss = size_loss(targets * target_scale_factor, preds_verts, eps=eps) + complex_cosine_loss(
...
if train:
    ...
    for epoch in range(n_epoch):
        per_vertex_train_loss, train_loss, train_preds, train_targets, train_frames_verts, train_frames_faces, vert2face_apply_transport, train_loss_array = train_epoch(
            epoch, normalize_targets)

        writer.add_scalar('training loss', np.log(np.array(train_loss_array).mean()), epoch * len(train_loader))

        if epoch % test_every == 0:
            per_vertex_test_loss, test_loss, test_preds, test_targets, test_frames_verts, test_frames_faces, vert2face_apply_transport, test_loss_array = test(
                normalize_targets)
