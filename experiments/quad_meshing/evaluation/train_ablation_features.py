...
from vector_heat_net.geometry_vector import global_to_local_batch
...
DATASET = "HEADS"
pretrain_path = os.path.join(base_path, "output/{}/saved_models/{}.pth".format("20240104_0755", "4480"))
...
if train:
    ...
    shutil.copy2(os.path.join(base_path, "train_ablation_features.py"),
                 os.path.join(scripts_path, "train_ablation_features.py"))
    shutil.copy2(os.path.join("src/vector_heat_net/data/retopo_dataset.py"),
...
# === Create the model
C_in = {'xyz': 3, 'gaussian_curvature': 2, 'principal_curvature': 2, 'mean_curvature': 2, 'hks_plus_principal': 15,
        'hks_plus_gaussian': 15, 'hks_grad': 15, 'hks_grad_cograd': 15, 'hks_normalized': 15, 'random': 3}[
    input_features]  # dimension of input features
...
def train_epoch(epoch, normalize_targets=False):
    ...
    loss_array = []
    size_loss_array = []
    direction_loss_array = []

    for data in tqdm(train_loader):

        # Get data
        verts, faces, frames_verts, frames_faces, mass, L, evals, evecs, gradX, gradY, div, cotan_L, cotan_evals, cotan_evecs, targets, pd1, pd2, pv1, pv2 = data
        ...
        if input_features == 'xyz':
            ...
            #             print(torch.mean(torch.linalg.norm(grad_features, dim=0), dim=0))
        ...
        #             vec_features = torch.stack((pd1, pd2, pv1_grad, pv2_grad, pv1_grad_rotated, pv2_grad_rotated), dim=1).squeeze(2)[None, :, :]
            ...
            curvature_features = torch.stack((pd1, pd2, pv1_grad, pv2_grad, pv1_grad_rotated, pv2_grad_rotated),
                                             dim=1).squeeze(2)[None, :, :]
        ...
        size_loss_ = size_loss(targets, preds_verts, eps=eps)
        complex_cosine_loss_ = complex_cosine_loss(targets_normalized ** n_vectors, preds_verts ** n_vectors, eps)
        per_vertex_loss = size_loss_ + complex_cosine_loss_
        ...
        size_loss_array.append(size_loss_.detach().cpu().numpy())
        direction_loss_array.append(complex_cosine_loss_.detach().cpu().numpy())
        loss_array.append(loss.detach().cpu().numpy())
    return per_vertex_loss, loss, preds_verts, targets, frames_verts, frames_faces, loss_array, size_loss_array, direction_loss_array
    ...
    with torch.no_grad():
        ...
        for data in tqdm(test_loader):
            ...
            if input_features == 'xyz':
                ...
                normalize_mean = torch.mean(torch.linalg.norm(scalar_features_grad, dim=3)[:, :, :, None], dim=1)[:,
                                 None, :, :]
                scalar_features_grad = scalar_features_grad / normalize_mean
...
if train:
    ...
    for epoch in range(n_epoch):
        per_vertex_train_loss, train_loss, train_preds, train_targets, train_frames_verts, train_frames_faces, train_loss_array, train_size_loss_array, train_direction_loss_array = train_epoch(
            epoch, normalize_targets)

        writer.add_scalar('training loss', np.array(train_loss_array).mean(), epoch * len(train_loader))
        writer.add_scalar('training loss std', np.array(train_loss_array).std(), epoch * len(train_loader))

        if epoch % test_every == 0:
            per_vertex_test_loss, test_loss, test_preds, test_targets, test_frames_verts, test_frames_faces, test_loss_array, test_size_loss_array, test_direction_loss_array = test(
                normalize_targets)
            writer.add_scalar('test loss', np.array(test_loss_array).mean(), epoch * len(train_loader))
            writer.add_scalar('test size loss', np.array(test_size_loss_array).mean(), epoch * len(train_loader))
            writer.add_scalar('test direction loss', np.array(test_direction_loss_array).mean(),
                              epoch * len(train_loader))
            writer.add_scalar('test size loss std', np.array(test_size_loss_array).std(), epoch * len(train_loader))
            writer.add_scalar('test direction loss std', np.array(test_direction_loss_array).std(),
                              epoch * len(train_loader))
        ...
        if epoch > 0 and epoch % save_every == 0:
            ...
            output_filepath = os.path.join(base_path,
                                           f"output/{DATASET}/{formatted_datetime}/pred_json/output_train_{epoch}.json")
            ...
            output_filepath = os.path.join(base_path,
                                           f"output/{DATASET}/{formatted_datetime}/pred_json/output_test_{epoch}.json")
            with open(output_filepath, "w") as f:
                json.dump(
                    {
    ...
    per_vertex_test_loss, test_loss, test_preds, test_targets, test_frames_verts, test_frames_faces, test_loss_array = test(
        normalize_targets)
