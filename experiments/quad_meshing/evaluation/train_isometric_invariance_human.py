...
from vector_heat_net.utils import toNP, sparse_torch_to_np
from vector_heat_net.loss import complex_mse_loss, complex_nmse_loss, complex_cosine_loss, size_loss
from vector_heat_net.layers_vector_isometric_debug import complex_to_interleaved
from vector_heat_net.connection_laplacian.compute_hks import compute_hks
...
# parser.add_argument("--input_features", type=str, help="what features to use as input ('xyz' or 'hks') default: hks", default='gaussian_curvature')
...
if train:
    ...
    shutil.copy2(os.path.join(base_path, "train_isometric_invariance_human.py"), os.path.join(scripts_path, "train_rotation_invariance.py"))
...
train_dataset = RetopoInferenceDataset(dataset_path, split='train', k_eig=k_eig, use_cache=False, op_cache_dir=None)
train_loader = DataLoader(train_dataset, batch_size=None, shuffle=True)

test_dataset = RetopoInferenceDataset(dataset_path, split='test', k_eig=k_eig, use_cache=False, op_cache_dir=None)
...
for data in train_loader:
    ...
    avg_edge_length = igl.avg_edge_length(verts.cpu().numpy(), faces.cpu().numpy())
    ...
    grads_rotated = grads_rotated / torch.linalg.norm(grads_rotated, axis=1).mean() * avg_edge_length
...
if input_features == 'hks_grad_cograd' or input_features == 'hks_normalized':
    model = vector_heat_net.layers_vector_isometric_debug.VectorDiffusionNet(C_in=C_in * 2,
                                                                  C_out=1,
                                                                  C_width=256,
                                                                  N_block=6,
    ...
    model = vector_heat_net.layers_vector_isometric_debug.VectorDiffusionNet(C_in=C_in * 2 + 6,
                                                                  C_out=1,
                                                                  C_width=256,
                                                                  N_block=6,
                                                                  last_activation=None,
    ...
    model = vector_heat_net.layers_vector_isometric_debug.VectorDiffusionNet(C_in=C_in * 2 + 2,
                                                                  C_out=1,
                                                                  C_width=256,
                                                                  N_block=6,
                                                                  last_activation=None,
    ...
    model = vector_heat_net.layers_vector_isometric_debug.VectorDiffusionNet(C_in=C_in,
                                                                  C_out=1,
                                                                  C_width=256,
                                                                  N_block=6,
                                                                  last_activation=None,
...
def train_epoch(epoch, normalize_targets=False):
    ...
    for data in tqdm(train_loader):
        ...
        if input_features == 'xyz':
            ...
            avg_edge_length = igl.avg_edge_length(verts.cpu().numpy(), faces.cpu().numpy())
            scalar_features = vector_heat_net.geometry_vector.compute_hks_autoscale(cotan_evals, cotan_evecs, C_in)
#             if TRAIN_HKS is None:
#                 scalar_features = compute_hks(sparse_torch_to_np(cotan_L), mass.cpu().numpy(), num_times=15)
#                 scalar_features = torch.Tensor(scalar_features).to(device)
#                 TRAIN_HKS = scalar_features
#             else:
#                 scalar_features = TRAIN_HKS
            scalar_features = scalar_features.to(dtype=torch.float32)
            
#             SCALAR_SCALE = 1
#             SCALAR_SCALE = 2 * avg_edge_length / scalar_features.max()
#             scalar_features = scalar_features * SCALAR_SCALE
...
#             vec_features = grad_features[None, ...]
            ORIGINAL_FEATURES = grad_features
        elif input_features == 'hks_plus_principal':
            scalar_features = vector_heat_net.geometry_vector.compute_hks_autoscale(cotan_evals, cotan_evecs, C_in)
            scalar_features = scalar_features.to(dtype=torch.float32)
...
#                     scalar_features = compute_hks(sparse_torch_to_np(cotan_L), mass.cpu().numpy(), num_times=15)
#                     scalar_features = torch.Tensor(scalar_features).to(device)
#                     TEST_HKS = scalar_features
#                 else:
#                     scalar_features = TEST_HKS
                

                scalar_features = ORIGINAL_SCALARS
...
# #         print(TRAIN_L)
# #         print(TEST_L)
#         L_error = TRAIN_L.cpu().to_dense() - TEST_L.cpu().to_dense()
#         print("REAL\n")
#         print(f"NUM W/ ERROR > {threshold}: ", np.count_nonzero(L_error.real > threshold))
#         print("MAX ERROR: ", L_error.real.max())
#         print("MIN ERROR: ", L_error.real.min())
#         print("MEAN ERROR: ", L_error.real.mean())
        
#         print("IMAG\n")
#         print(f"NUM W/ ERROR > {threshold}: ", np.count_nonzero(L_error.imag > threshold))
#         print("MAX ERROR: ", L_error.imag.max())
#         print("MIN ERROR: ", L_error.imag.min())
#         print("MEAN ERROR: ", L_error.imag.mean())
