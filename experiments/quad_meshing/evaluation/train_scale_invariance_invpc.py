...
parser.add_argument("--input_features", type=str, help="what features to use as input ('xyz' or 'hks') default: hks", default='inv_principal_curvature')
...
DATASET = "ICML/scale_invariance_invpc"
...
if train:
    dataset_path = "/home/jovyan/git/vector-diffusion-net/experiments/quad_meshing/data/ICML/scale_invariance"
else:
    dataset_path = "/home/jovyan/git/vector-diffusion-net/experiments/quad_meshing/data/ICML/scale_invariance"
...
if train:
    ...
    shutil.copy2(os.path.join(base_path, "train_scale_invariance_invpc.py"), os.path.join(scripts_path, "train_scale_invariance_invpc.py"))
...
# === Create the model
C_in = {'xyz': 3, 'gaussian_curvature': 2, 'inv_principal_curvature': 2, 'principal_curvature': 6, 'hks_plus_principal': 15, 'hks_plus_gaussian': 15, 'hks_grad': 15, 'hks_grad_cograd': 15, 'random': 3}[input_features]  # dimension of input features
...
def train_epoch(epoch, normalize_targets=False):
    ...
    for data in tqdm(train_loader):
        ...
        elif input_features == 'inv_principal_curvature': 
            eps = torch.Tensor([1e-8]).to(device)

            # gradient after diffusion
            pd1 = torch.view_as_complex(pd1.to(dtype=torch.float32))[:, None] * (1.0 / torch.maximum(eps, pv1))
            pd2 = torch.view_as_complex(pd2.to(dtype=torch.float32))[:, None] * (1.0 / torch.maximum(eps, pv2))
