...
for n_layers in [4, 8]:
    ...
    DATASET = "ICML/SENSITIVITY_ANALYSIS"
    pretrain_path = os.path.join(base_path, "output/{}/{}/saved_models/{}.pth".format(DATASET, "20240201_2221", "27200"))

    experiment_directory_path = os.path.join(base_path, f"output/{DATASET}/{n_layers}_LAYERS/")
    ...
    tb_path = os.path.join(base_path, f"output/{DATASET}/tb/{n_layers}_LAYERS")
    ...
    if train:
        ...
        shutil.copy2(os.path.join(base_path, "train_sensitivity_analysis.py"), os.path.join(scripts_path, "train.py"))
    ...
    def train_epoch(epoch, normalize_targets=False):
        ...
        for data in tqdm(train_loader):
            ...
            if input_features == 'hks_grad_cograd':
                ...
                scalar_features_grad = scalar_features_grad / torch.std(torch.linalg.norm(scalar_features_grad, axis=3), dim=1)[:, None, :, None]
    ...
    if train:
        ...
        for epoch in range(n_epoch):
            ...
            writer.add_scalar('training loss', np.array(train_loss_array).mean(), epoch)
            ...
            if epoch % test_every == 0:
                ...
                writer.add_scalar('test loss', np.array(test_loss_array).mean(), epoch)
    ...
    #                            np.squeeze(train_grad_features).imag[:, None] * train_frames_verts[:, 1, :]
                ...
                output_filepath = os.path.join(base_path, f"output/{DATASET}/{n_layers}_LAYERS/pred_json/output_train_{epoch}.json")
    ...
    #                np.squeeze(test_grad_features).imag[:, None] * test_frames_verts[:, 1, :]
                ...
                output_filepath = os.path.join(base_path, f"output/{DATASET}/{n_layers}_LAYERS/pred_json/output_test_{epoch}.json")
                with open(output_filepath, "w") as f:
                    json.dump(
                        {
