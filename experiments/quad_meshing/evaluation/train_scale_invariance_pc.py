...
parser.add_argument("--input_features", type=str, help="what features to use as input ('xyz' or 'hks') default: hks", default='principal_curvature')
...
DATASET = "ICML/scale_invariance_pc"
...
if train:
    ...
    shutil.copy2(os.path.join(base_path, "train_scale_invariance_pc.py"), os.path.join(scripts_path, "train_scale_invariance_pc.py"))
...
# === Create the model
C_in = {'xyz': 3, 'gaussian_curvature': 2, 'principal_curvature': 2, 'hks_plus_principal': 15, 'hks_plus_gaussian': 15, 'hks_grad': 15, 'hks_grad_cograd': 15, 'random': 3}[input_features]  # dimension of input features
...
def train_epoch(epoch, normalize_targets=False):
    ...
    for data in tqdm(train_loader):
        ...
        if input_features == 'xyz':
            ...
            max_curvature = torch.maximum(torch.linalg.norm(pd1, dim=1).max(), torch.linalg.norm(pd2, dim=1).max())
            inv_max_curvature = 1 / max_curvature * 100

            pd1 = pd1 / torch.linalg.norm(pd1, dim=1).mean() * inv_max_curvature
            pd2 = pd2 / torch.linalg.norm(pd2, dim=1).mean() * inv_max_curvature
