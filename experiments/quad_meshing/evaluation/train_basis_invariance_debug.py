...
from vector_heat_net.geometry_vector import local_to_global_batch, global_to_local_batch
...
rotate_vector_field_operator = torch.exp((0.0+1.0j) * torch.tensor([torch.pi / 2])).to(device)
...
pretrain_path = os.path.join(base_path, "output/{}/{}/saved_models/{}.pth".format(DATASET, "20240201_0818", "10000"))
...
#             pd2 = pd2 / torch.linalg.norm(pd2, dim=1).mean()
            ...
            pd1_local = torch.view_as_complex(pd1_local.to(dtype=torch.float32))[:, None]            
