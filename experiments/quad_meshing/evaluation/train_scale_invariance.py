...
if train:
    ...
    shutil.copy2(os.path.join(base_path, "train_scale_invariance.py"), os.path.join(scripts_path, "train_scale_invariance.py"))
...
# === Create the model
C_in = {'xyz': 3, 'gaussian_curvature': 2, 'principal_curvature': 6, 'hks_plus_principal': 15, 'hks_plus_gaussian': 15, 'hks_grad': 15, 'hks_grad_cograd': 15, 'random': 3}[input_features]  # dimension of input features
...
if train:
    ...
    for epoch in range(n_epoch):
        per_vertex_train_loss, train_loss, train_preds, train_targets, train_frames_verts, train_frames_faces, vert2face_apply_transport, train_loss_array, train_grad_features = train_epoch(epoch, normalize_targets)
